...
function getRandomData(type) {
  ...
  // // Function takes an object legnth and returns a random number in a range of that length parameter 
  function rn(len) {
...
function clearAll() {
    ...
    pShowJoke.textContent = "";
    pShowQuote.textContent = "";
    pShowRiddle.textContent = "";
    pShowAnswer.textContent = "";
  }
  ...
  const pMeme = document.getElementById("pMeme");
  const parent = document.getElementById("pMeme").parentNode;
  const img = document.createElement("img");
  parent.insertBefore(img, pMeme.nextSibling);
  img.style.display = 'none';
  ...
  function showMeme() {
    ...
    img.alt = "sorry, could not load an image :(";
  ...
  const pShowJoke = document.createElement("p");
  parent.insertBefore(pShowJoke, pJoke.nextSibling);
  
  function showJoke() {
    // Value is a string representing the joke
    const randomJokeText = getRandomData("jokes");
  
    pShowJoke.textContent = randomJokeText;
  }
  ...
  const pShowQuote = document.createElement("p");
  parent.insertBefore(pShowQuote, pQuote.nextSibling);
  
  function showQuote() {
    // Value should be in format: { quote: '', author: '' }
    const randomQuote = getRandomData("quotes");
    pShowQuote.textContent = `${randomQuote.quote} ${'"'+randomQuote.author+'"'}`;
  }
  ...
  const pRiddle = document.getElementById("pRiddle");
  const pShowRiddle = document.createElement("p");
  parent.insertBefore(pShowRiddle, pRiddle.nextSibling);
  
  function showRiddle() {
    // Value should be in format: { question: '', answer: '' }
    const randomRiddle = getRandomData("riddles");
    pShowRiddle.textContent = randomRiddle.question;
    pShowAnswer.textContent = "";
  }
  ...
  const pShowAnswer = document.createElement("p");
  parent.insertBefore(pShowAnswer, pAnswer.nextSibling);
  
  function revealAnswers() {
    const riddleAnswer = getRandomData("riddles");     
    if (
      pShowRiddle.innerHTML.length !== 0 &&
      pShowAnswer.innerHTML.length !== 0
    ) {
      alert("Answer is already revealed");
    } else if (pShowRiddle.innerHTML.length == 0) {
      alert("There is no riddle");
    } else if (
      pShowRiddle.innerHTML.length !== 0 &&
      pShowAnswer.innerHTML.length == 0
    ) {
      pShowAnswer.textContent = "Answer: "+riddleAnswer.answer;
    }
  }
